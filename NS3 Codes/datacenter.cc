...
// Enhanced statistics printing - FIXED: Pass FlowMonitorHelper by reference
void PrintEnhancedStatistics(Ptr<FlowMonitor> monitor, 
                            FlowMonitorHelper& flowmonHelper,  // Changed to reference
...
int main(int argc, char *argv[]) {
    ...
    for (uint32_t i = 0; i < nAgg; i++) {
        // Each aggregation switch connects to half of the ToR switches
        uint32_t torsPerAgg = nTor / nAgg;
        for (uint32_t j = 0; j < torsPerAgg; j++) {
            uint32_t torIndex = i * torsPerAgg + j;
    ...
    // Output flow statistics - FIXED: Pass helper by reference
